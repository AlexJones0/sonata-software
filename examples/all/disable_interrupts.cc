...
[[gnu::noinline]] [[cheri::interrupt_state(disabled)]] int run_without_interrupts(int x) {
	return x + 1;
}

[[gnu::noinline]] [[cheri::interrupt_state(disabled)]] void also_without_interrupts(int *x) {
	*x = 1;
}
